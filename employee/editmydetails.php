...
//fetching information belonging to this user
	 $query = mysqli_query($dbc,"SELECT * FROM registration WHERE staffid='$staffid'");
...
if(!empty($firstname)&& !empty($surname) && !empty($lastname) && !empty($idno) && !empty($staffid)
	...
	$updatesql=mysqli_query($dbc,"UPDATE registration SET firstname='$firstname', surname='$surname', lastname='$lastname',idno='$idno', department='$department', supervisor='$supervisor',
